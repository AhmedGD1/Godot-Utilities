...
namespace Stately
{
    public partial class StateMachine<T> : IDisposable where T : Enum
    {
        ...
        public void UpdateIdle(double delta)
        {
            Process(FSMProcessMode.Idle, (float)delta);
        }

        public void UpdateFixed(double delta)
        {
            Process(FSMProcessMode.Fixed, (float)delta);
        }
